...
const getAllCategories = (filters, paginationOptions) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const { popular } = filters, filtersData = __rest(filters, ["popular"]);
    const andConditions = [];
    if (popular) {
    ...
    const total = yield categories_model_1.Category.countDocuments(whereConditions);
    const result = yield categories_model_1.Category.find(whereConditions)
        .sort(sortConditions)
        .skip(skip)
        .limit(limit);
    return {
